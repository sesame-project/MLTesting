...
def mass_activation_plot(unsup_data, zero_shot_data, sup_data, data_dict):
    ...
    plotly.offline.plot(fig,
                        filename=os.path.join(data_dict["visualize"]["plot_directory"], "mass_activation_plot.pdf"),
                        auto_open=False)
    fig.show()
...
def freq_analysis_plot(sup_data, unsup_data_epoch1, unsup_data_epoch49, data_dict):
    ...
    df_['unsup epoch 1 act. mass %'] = df_['unsup1-mass_activation'].apply(
        lambda x: x / df_['unsup1-mass_activation'].sum())
    df_['unsup epoch 49 act. mass %'] = df_['unsup49-mass_activation'].apply(
        lambda x: x / df_['unsup49-mass_activation'].sum())
    ...
    fig.add_trace(go.Bar(x=list(plot_dict['unsup epoch 49 act. mass %'].keys()),
                         ...
                         name="unsup epoch 49 act. mass %",
                         marker_color=data_dict['visualize']['viz_colors']['unsup_epoch_49']))
    ...
    # fig.write_image(os.path.join(data_dict["visualize"]["plot_directory"], "mass_activation_plot.pdf"))
    plotly.offline.plot(fig,
                        filename=os.path.join(data_dict["visualize"]["plot_directory"], "freq_activation_plot.pdf"),
                        auto_open=False)
    fig.show()
...
def plot_top_10_hellinger_neurons(hellinger_stats, model1_data, model2_data, color1, color2, modelname1, modelname2,
    ...
    for neuron in top_10_neurons:
        ...
        if process_data_flag == True:
            ...
            model1_top_20_activation_index = sorted(range(len(temp_activations)), key=lambda x: temp_activations[x])[
                                             -n_tokens:]
            ...
            for key, value in model2_dict.items():
                for index in range(len(value['POS'])):
                    temp_tokens.append(key)
                    temp_activations.append(value['activation'][index])
            model2_top_20_activation_index = sorted(range(len(temp_activations)), key=lambda x: temp_activations[x])[
                                             -n_tokens:]
        ...
        model1_value = [[(value[0], np.nan) if value[1] == 0.0 else (value[0], value[1]) for value in pairs] for pairs
                        in values_model1]
        model2_value = [[(value[0], np.nan) if value[1] == 0.0 else (value[0], value[1]) for value in pairs] for pairs
                        in values_model2]
        ...
        if process_data_flag == True:
            ...
            for key, value in model1_dict.items():
                for index in range(len(value['POS'])):
                    temp_tokens.append(key)
                    temp_activations.append(value['activation'][index])
            model1_least_20_activation_index = sorted(range(len(temp_activations)), key=lambda x: temp_activations[x])[
                                               -n_tokens:]
            ...
            for key, value in model2_dict.items():
                for index in range(len(value['POS'])):
                    temp_tokens.append(key)
                    temp_activations.append(value['activation'][index])
            model2_least_20_activation_index = sorted(range(len(temp_activations)), key=lambda x: temp_activations[x])[
                                               -n_tokens:]
