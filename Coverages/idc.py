...
def quantizeSilhouette(train_layer_outs, conv, relevant_neurons,subsetTop,model):
    quantized_ = []
    Clusters = []
...
def measure_idc(model, model_name, test_inputs, subject_layer,
                                   ...
                                   test_layer_outs, qtized, is_conv,subsetTop,
    ...
    for test_idx in range(len(test_inputs)):
            ...
            for r in relevant_neurons:
                ...
                if 'conv' in model.layers[neuron_layer].name:
                    ...
                    is_conv = False
                if is_conv:
                # if r in list(test_layer_outs[test_idx][...,r]):
                # for l in test_layer_outs:
                #     print(test_layer_outs[subject_layer][test_idx][...,r])
                #     lout.append(np.mean(test_layer_outs[test_idx][...,r]))
                     lout.append(np.mean(test_layer_outs[neuron_layer][test_idx][..., r]))
                else:
                    # lout = test_layer_outs[test_idx][relevant_neurons]
                    lout = test_layer_outs[neuron_layer][test_idx][relevant_neurons]
